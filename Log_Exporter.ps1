...
while ($true)
{
    ...
    $events = Get-WinEvent -FilterXml $XMLFilter -MaxEvents 500
    if ($Error) {
        Write-Output "No Failed Logons found. Re-run script when a login has failed."
    }
    ...
    foreach ($event in $events) {
        ...
        if ($event.properties[19].Value.Length -ge 5) {
            ...
            if (-Not ($log_contents -match "$($timestamp)") -or ($log_contents.Length -eq 0)) {
                ...
                $QUERY = " https://api.ip2location.io/?ip=$($sourceIp)"
                $response = Invoke-WebRequest -UseBasicParsing -Uri $QUERY
                ...
                Write-Output "Event already exists in the custom log. Skipping." -ForegroundColor Gray -BackgroundColor Black
            }
        }
    }
}
